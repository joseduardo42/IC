...
provide the initial conditions in the components of circuits and also the 
...
f1 = 100
f2 = 8
vc0 = -0.1771291668869918
tf = 1 / f2
N = int(100 * f1 / f2)
(t_sim_trans, deltat) = np.linspace(0, tf, N, retstep=True)

vs = Vm1 * np.sin(2 * pi * f1 * t_sim_trans[0]) + Vm2 * np.sin(
    2 * pi * f2 * t_sim_trans[0])  # voltage in source in actual time (t=0)
...
i0 = float(x[4])  # current in t0, to use in interactions in
...
# storage the parameters of circuits at t0
